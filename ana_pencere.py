...
class AnaPencere(QMainWindow):
    def __init__(self, socket_name="SistemAsistaniInstance"):
        ...
        # PyInstaller ile derlenince sys.executable çalışılan klasörü verir (/opt/sistem-asistani/)
        ...
        # İkonu Yükle (Eğer dosya bulunamazsa varsayılan ikon oluşur)
        if os.path.exists(self.icon_path_light):
            self.app_icon = QIcon(self.icon_path_light)
        else:
            # Yedek: Eğer /opt altında bulamazsa yerel klasöre bakar
    ...
    def pencereyi_one_getir(self, socket):
        _ = socket.readAll()
        if self.hud_window and self.hud_window.isVisible(): self.hud_window.close() 
        self.showNormal()
        self.ekran_ortala()
        self.activateWindow()
        self.raise_()
